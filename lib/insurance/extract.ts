...
const OUTPUT_SCHEMA = {
  ...
  required: ["company_name", "policies"],
  properties: {
    company_name: {
...
const PROMPT = `This file should be a certificate of insurance (often an ACORD 25 form) for one of our subcontractors. Read it and extract:

1. company_name — the INSURED's business name (the subcontractor). Not the producer/agency and not the certificate holder.
2. policies — one entry per policy listed. Map coverage rows to types:
