...
- Propose the concrete action(s) the note implies. Typical mappings:
  ...
  - The homeowner asking for EXTRA or CHANGED work is a CHANGE ORDER — recognize it even when nobody says "change order": "the client wants to add a covered patio", "owner asked to swap carpet for hardwood in the bedrooms", "they want a third garage bay — need to price it", "write up a CO for the extra can lights". → propose_create_decision with decision_kind='change_order', a short title naming the change, and a description capturing the requested scope plus any dollar amount or timing mentioned. It starts as a DRAFT for staff to price — do NOT also propose sending it to the client unless the user explicitly says to. If the note is instead about the client choosing among planned options (finishes, fixtures), that's a selection, not a change order.
