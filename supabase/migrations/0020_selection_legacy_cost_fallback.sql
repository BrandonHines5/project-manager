-- Fix legacy fallback in the allowance approval branch of
-- client_decide_decision. The previous version used
-- coalesce(v_choice_price, 0) - allowance_amount, which turned legacy
-- allowance selections (where the chosen choice has no recorded price)
-- into a credit equal to the entire allowance. Mirror the non-allowance
-- branch's behaviour: when the chosen choice has no price, preserve the
-- existing cost_delta instead.
    ...
    if v_decision.kind = 'selection' then
      ...
      update public.decisions
        set status = 'approved',
            ...
            -- With allowance: variance = chosen.price - allowance, but if
            --   chosen.price is null (legacy data) preserve the existing
            --   cost_delta rather than booking a full-allowance credit.
            -- Without allowance: chosen.price IS the cost_delta. Same legacy
            --   fallback applies.
            cost_delta = case
              when v_decision.allowance_amount is not null then
                case
                  when v_choice_price is null then cost_delta
                  else v_choice_price - v_decision.allowance_amount
                end
              else
